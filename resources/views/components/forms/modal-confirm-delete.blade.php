@props([
    'id' => 'delete-modal',   // ID único del modal
    'title' => 'Confirmar eliminación',
    'message' => '¿Estás seguro de eliminar este registro?',
    'routeBase' => '',        // base de la ruta (ej: 'usuarios')
    'itemId' => null,         // ID del recurso
    'confirmText' => 'Sí, eliminar',
    'cancelText' => 'Cancelar'
])

<div id="{{ $id }}" class="fixed hidden inset-0 z-50 justify-center items-center size-auto max-h-none max-w-none overflow-y-auto">
  <!-- Fondo oscuro -->
  ...
  <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
    <div class="relative transform overflow-hidden rounded-lg bg-slate-50 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg dark:bg-gray-800 outline-1 outline-slate-50/10">
      ...
      <div class="bg-slate-50 dark:bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
        <div class="sm:flex sm:items-start">
          ...
          <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
            ...
            <div class="mt-2">
              <p id="{{ $id }}-message" class="text-base text-gray-500 dark:text-gray-400">
                {{ $message }}
              </p>
            </div>
          </div>
      ...
      <div class="bg-gray-50 dark:bg-gray-700/25 px-4 py-3 sm:flex sm:flex-row-reverse gap-4">
        ...
        <button type="button" onclick="closeModal()" class="inline-flex w-full justify-center rounded-md px-3 py-2 bg-slate-50 dark:bg-slate-50/10 text-sm font-semibold text-gray-900 dark:text-slate-50 shadow-xs hover:bg-gray-50 dark:hover:bg-slate-50/20 sm:w-auto">
          {{ $cancelText }}
        </button>
      </div>
    </div>
