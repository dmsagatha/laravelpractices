...
<!-- Modal genérico -->
<div id="{{ $id }}-modal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black bg-opacity-50">
  <div class="w-full max-w-md rounded-lg bg-white p-6 shadow-lg dark:bg-slate-800">
    ...
    <div class="flex justify-end gap-3">
      ...
      <form method="POST" id="{{ $id }}-form">
        ...
        @method('DELETE')
        <button type="submit"
          class="shadow-xs inline-flex w-full justify-center rounded-md bg-red-600 px-3 py-2 text-sm font-semibold text-slate-50 hover:bg-red-500 sm:w-auto dark:bg-red-500 dark:hover:bg-red-400">
          {{ $confirmText }}
        </button>
      </form>
    </div>
